...
export function DiscordBotCommandHub({
  ...
  const [activeTab, setActiveTab] = useState<
    'setup' | 'requests' | 'generate' | 'show' | 'verify' | 'inspect' | 'manage' | 'code'
  >('setup');
  ...
  // Server Setup State (/card setup)
  const [setupConfig, setSetupConfig] = useState<GuildSetupConfig | null>(null);
  const [loadingSetup, setLoadingSetup] = useState<boolean>(false);
  const [staffChannelInput, setStaffChannelInput] = useState<string>('staff-applications');
  const [deliveryChannelInput, setDeliveryChannelInput] = useState<string>('citizen-cards');
  const [staffRoleInput, setStaffRoleInput] = useState<string>('Review Officer');
  const [autoNicknameInput, setAutoNicknameInput] = useState<boolean>(true);
  const [setupSaving, setSetupSaving] = useState<boolean>(false);
  const [setupFeedback, setSetupFeedback] = useState<string | null>(null);

  // Staff Review Queue State
  const [pendingRequests, setPendingRequests] = useState<CardRequestRecord[]>([]);
  const [loadingRequests, setLoadingRequests] = useState<boolean>(false);
  const [actingRequestId, setActingRequestId] = useState<string | null>(null);
  const [requestFilter, setRequestFilter] = useState<'ALL' | 'PENDING' | 'APPROVED' | 'DECLINED'>('PENDING');
  const [declineModal, setDeclineModal] = useState<{ id: string; name: string } | null>(null);
  const [declineReasonInput, setDeclineReasonInput] = useState<string>('Invalid or unverified Roblox identity');
  ...
  const fetchSetup = async () => {
    ...
    try {
      const resp = await fetch('/api/card/setup/status');
      if (resp.ok) {
        const data = await resp.json();
        setSetupConfig(data.config || null);
        if (data.config?.staffChannelName) setStaffChannelInput(data.config.staffChannelName);
        if (data.config?.deliveryChannelName) setDeliveryChannelInput(data.config.deliveryChannelName);
        if (data.config?.staffRoleName) setStaffRoleInput(data.config.staffRoleName);
        if (data.config?.autoNickname !== undefined) setAutoNicknameInput(data.config.autoNickname);
      }
    } catch (err) {
  ...
  const fetchRequests = async () => {
    ...
    try {
      const resp = await fetch('/api/card/requests');
      if (resp.ok) {
        const data = await resp.json();
  ...
  // Handle Save Setup (/card setup)
  const handleSaveSetup = async () => {
    setSetupSaving(true);
    setSetupFeedback(null);
    try {
      const resp = await fetch('/api/card/setup', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          staffChannelId: '998877665544332211',
          staffChannelName: staffChannelInput,
          deliveryChannelId: deliveryChannelInput ? '112233445566778899' : null,
          deliveryChannelName: deliveryChannelInput || null,
          staffRoleId: staffRoleInput ? '445566778899001122' : null,
          staffRoleName: staffRoleInput || null,
          autoNickname: autoNicknameInput,
          setupBy: executor,
        }),
      });
      const data = await resp.json();
      if (resp.ok) {
        ...
        setSetupFeedback('✅ Server configuration saved! All UOI commands are now unlocked.');
        setTimeout(() => setSetupFeedback(null), 4500);
      } else {
        setSetupFeedback(`❌ Failed to save setup: ${data.error}`);
      }
    } catch (err: any) {
      setSetupFeedback(`❌ Error saving setup: ${err.message}`);
    } finally {
      setSetupSaving(false);
    }
  };

  // Submit via Citizen Workflow (Routes to Staff Review Queue)
  const handleSubmitCardRequest = async () => {
    setDuplicateWarning(null);
    try {
      const resp = await fetch('/api/card/request/submit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          targetUser: {
          ...
          robloxAvatarUrl: getStoredImage('avatar') || null,
        }),
      });

      const json = await resp.json();

      if (resp.status === 403 && json.error === 'SERVER_NOT_CONFIGURED') {
        setDuplicateWarning({
          message:
            '⚠️ Server is locked: An administrator must configure /card setup before citizens can submit card applications.',
        });
        return;
      }
      ...
      if (resp.status === 409) {
        setDuplicateWarning({
          message: json.message || 'Citizen already has an active card or pending application.',
          existingCard: json.existingCard,
        });
        return;
      }
  ...
  // Staff Action: Accept & Issue Card
  const handleAcceptRequest = async (requestId: string) => {
    setActingRequestId(requestId);
    setRequestFeedback(null);
    try {
      const resp = await fetch('/api/card/request/accept', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          requestId,
      ...
      const data = await resp.json();
      if (resp.ok) {
        setRequestFeedback(`✅ Approved & Issued card for ${data.card.issuedTo.fullName} (${data.card.serialId})!`);
        fetchRequests();
        fetchVault();
        setTimeout(() => setRequestFeedback(null), 4500);
      } else {
        setRequestFeedback(`❌ Error accepting request: ${data.error}`);
      }
    } catch (err: any) {
      setRequestFeedback(`❌ Error accepting request: ${err.message}`);
    } finally {
      setActingRequestId(null);
    }
  };

  // Staff Action: Decline with Reason
  const handleDeclineRequest = async (requestId: string, reason: string) => {
    setActingRequestId(requestId);
    setRequestFeedback(null);
    try {
      const resp = await fetch('/api/card/request/decline', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          requestId,
      ...
      const data = await resp.json();
      if (resp.ok) {
        setRequestFeedback(`❌ Declined application for ${data.request?.fullName || 'citizen'}. Reason: "${reason}"`);
        fetchRequests();
        setDeclineModal(null);
        setTimeout(() => setRequestFeedback(null), 4500);
      } else {
        setRequestFeedback(`❌ Error declining request: ${data.error}`);
      }
    } catch (err: any) {
      setRequestFeedback(`❌ Error declining request: ${err.message}`);
    } finally {
      setActingRequestId(null);
    }
  };
...
client.on('interactionCreate', async (interaction) => {
  ...
  // Handle staff review buttons (Accept & Decline)
  ...
  // Handle staff decline modal submission (Reason)
  if (interaction.isModalSubmit()) {
    if (interaction.customId.startsWith('card_modal_')) {
      try {
        await cardCommand.handleModal(interaction);
      } catch (err) {
        console.error('Error handling modal interaction:', err);
      }
    }
    return;
  }
...
`;
  ...
  return (
    <div id="discord-command-hub" className="bg-slate-900/90 border border-slate-800 rounded-xl p-5 shadow-sm flex flex-col gap-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-slate-800">
        ...
        <div className="flex flex-wrap items-center gap-1 bg-slate-950 p-1 rounded-lg border border-slate-800 text-xs">
          <button
            ...
            className={`px-2.5 py-1 rounded transition-colors font-medium cursor-pointer flex items-center gap-1.5 ${
              activeTab === 'setup'
                ? 'bg-amber-600 text-white shadow-sm'
                : setupConfig?.isSetup
                ? 'text-amber-400/90 hover:text-amber-200'
                : 'text-rose-400 hover:text-rose-300 font-semibold'
            }`}
          >
            {setupConfig?.isSetup ? <Unlock className="w-3 h-3 text-emerald-400" /> : <Lock className="w-3 h-3 text-rose-400" />}
            <span>/card setup</span>
          </button>
          <button
            onClick={() => setActiveTab('requests')}
            className={`px-2.5 py-1 rounded transition-colors font-medium cursor-pointer flex items-center gap-1.5 ${
              activeTab === 'requests' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
      ...
      {/* Setup Required Warning Banner */}
      {setupConfig && !setupConfig.isSetup && (
        <div className="bg-rose-950/40 border border-rose-500/60 rounded-lg p-3.5 flex flex-wrap items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-2.5">
            <Lock className="w-5 h-5 text-rose-400 shrink-0" />
            <div>
              <div className="font-bold text-rose-200">Server Commands Locked (/card setup required)</div>
              <div className="text-rose-300/80 text-[11px]">
                The server has not been configured yet. All card commands stay locked/hidden in Discord until an administrator runs <code className="bg-rose-900/60 px-1 py-0.5 rounded text-rose-100">/card setup</code>.
              </div>
            </div>
          </div>
          <button
            onClick={() => setActiveTab('setup')}
            className="px-3 py-1.5 bg-rose-600 hover:bg-rose-500 text-white rounded font-medium text-xs flex items-center gap-1.5 transition-colors shrink-0 cursor-pointer"
          >
            <Settings className="w-3.5 h-3.5" />
            <span>Configure Server Now</span>
          </button>
        </div>
      )}
      ...
      {requestFeedback && (
        <div className="bg-indigo-950/50 border border-indigo-500/50 rounded-lg p-2.5 text-xs text-indigo-200 flex items-center gap-2">
          ...
          <div className="bg-slate-950 p-4 rounded-lg border border-slate-800 flex flex-col gap-3">
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800/80 pb-3">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h4 className="font-bold text-sm text-slate-200">Server Workflow Setup & Gatekeeper</h4>
                  <p className="text-[11px] text-slate-400">
                    Configures routing channels, review permissions, and unlocks commands for this server.
                  </p>
                </div>
              </div>
                ...
                >
                  {setupConfig?.isSetup ? (
                    <>
                      ...
                      <span>SERVER CONFIGURED (UNLOCKED)</span>
                    </>
                  ) : (
                    <>
                      <Lock className="w-3.5 h-3.5" />
                      <span>COMMANDS LOCKED (SETUP REQUIRED)</span>
                    </>
                  )}
                </span>
              </div>
            ...
            {setupFeedback && (
              <div className="p-3 rounded bg-amber-950/30 border border-amber-500/40 text-amber-200 text-xs flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs pt-1">
              ...
              <div className="flex flex-col gap-1">
                <label className="font-semibold text-slate-300 flex items-center justify-between">
                  <span>1. Staff Applications Channel (Required):</span>
                  <span className="text-[10px] text-amber-400 font-mono">Routes /card generate</span>
                </label>
                <input
                  type="text"
                  value={staffChannelInput}
                  onChange={(e) => setStaffChannelInput(e.target.value)}
                  placeholder="e.g. staff-applications or 998877665544332211"
                  className="bg-slate-900 border border-slate-800 rounded px-3 py-2 text-slate-200 font-mono text-xs focus:outline-none focus:border-amber-500"
                />
                <span className="text-[11px] text-slate-400">
                  Channel where pending citizen card requests arrive with interactive <strong>Accept</strong> and <strong>Decline</strong> buttons.
                </span>
              </div>
              ...
              <div className="flex flex-col gap-1">
                <label className="font-semibold text-slate-300 flex items-center justify-between">
                  <span>2. Delivery / Announcement Channel:</span>
                  <span className="text-[10px] text-slate-500 font-mono">Optional</span>
                </label>
                <input
                  type="text"
                  value={deliveryChannelInput}
                  onChange={(e) => setDeliveryChannelInput(e.target.value)}
                  placeholder="e.g. citizen-cards or 112233445566778899"
                  className="bg-slate-900 border border-slate-800 rounded px-3 py-2 text-slate-200 font-mono text-xs focus:outline-none focus:border-amber-500"
                />
                <span className="text-[11px] text-slate-400">
                  Channel where newly approved cards and decline notifications are posted for citizens.
                </span>
              </div>
              ...
              <div className="flex flex-col gap-1">
                ...
                <input
                  ...
                  placeholder="e.g. Review Officer or 445566778899001122"
                  className="bg-slate-900 border border-slate-800 rounded px-3 py-2 text-slate-200 font-mono text-xs focus:outline-none focus:border-amber-500"
                />
                <span className="text-[11px] text-slate-400">
                  Role permitted to accept or decline card applications. (Server Admins always have access).
                </span>
              </div>

              {/* Auto Nickname */}
              <div className="flex flex-col justify-between gap-1 bg-slate-900/60 p-3 rounded border border-slate-800/80">
                <div className="flex items-center justify-between">
                  <span className="font-semibold text-slate-300">4. Automatic Server Nickname:</span>
                  <input
                    type="checkbox"
                    checked={autoNicknameInput}
                    onChange={(e) => setAutoNicknameInput(e.target.checked)}
                    className="w-4 h-4 accent-amber-500 cursor-pointer"
                  />
                </div>
                <span className="text-[11px] text-slate-400">
                  When enabled, accepting a card updates the member's server profile nickname to <code className="text-emerald-400 font-mono">Full Name [Serial ID]</code>.
                </span>
              </div>
            </div>

            <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-slate-800/80 mt-1">
              <div className="text-[11px] text-slate-400">
                Equivalent Discord Slash Command: <code className="text-amber-300 font-mono bg-slate-900 px-2 py-0.5 rounded">/card setup staff_channel:#{staffChannelInput || 'staff-chat'} delivery_channel:#{deliveryChannelInput || 'cards'}</code>
              </div>

              <button
                onClick={handleSaveSetup}
                disabled={setupSaving}
                className="px-4 py-2 bg-amber-600 hover:bg-amber-500 disabled:opacity-50 text-white font-bold text-xs rounded-lg flex items-center gap-1.5 cursor-pointer transition-colors shadow-sm"
              >
                {setupSaving ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <CheckCircle2 className="w-3.5 h-3.5" />}
                <span>Save Setup & Unlock Server Commands</span>
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-slate-950 p-4 rounded-lg border border-slate-800 flex flex-col gap-3">
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800/80 pb-3">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h4 className="font-bold text-sm text-slate-200">Staff Review Queue</h4>
                  <p className="text-[11px] text-slate-400">
                    Incoming card applications from <code className="text-amber-400">/card generate</code> awaiting officer decision.
                  </p>
                </div>
              </div>
                ...
                {(['PENDING', 'APPROVED', 'DECLINED', 'ALL'] as const).map((filter) => (
                  <button
                    ...
                    className={`px-2 py-0.5 rounded cursor-pointer font-medium transition-colors ${
                      requestFilter === filter ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-slate-200'
                    }`}
                  >
                    {filter}
            ...
            {loadingRequests ? (
            ...
            ) : pendingRequests.filter((r) => requestFilter === 'ALL' || r.status === requestFilter).length === 0 ? (
              <div className="py-8 text-center text-xs text-slate-400 flex flex-col items-center gap-1.5">
                ...
                <span>No applications found under the "{requestFilter}" filter.</span>
                <button
                  onClick={() => setActiveTab('generate')}
                  className="text-indigo-400 hover:underline text-[11px] mt-1 cursor-pointer"
                >
                  Submit a test application in /card generate
                </button>
              </div>
            ) : (
              <div className="grid grid-cols-1 gap-3">
                {pendingRequests
                  ...
                  .map((req) => (
                    <div
                      key={req.id}
                      className="bg-slate-900 border border-slate-800 rounded-lg p-3.5 flex flex-col md:flex-row md:items-center justify-between gap-4 text-xs"
                    >
                      <div className="flex items-start gap-3">
                        {req.robloxAvatarUrl ? (
                          <img
                            src={req.robloxAvatarUrl}
                            alt={req.fullName}
                            className="w-12 h-12 rounded-lg object-cover bg-slate-800 border border-slate-700 shrink-0"
                          />
                        ) : (
                          <div className="w-12 h-12 rounded-lg bg-slate-800 border border-slate-700 flex items-center justify-center text-slate-400 shrink-0 font-bold">
                            {req.fullName[0] || 'U'}
                          </div>
                        )}
                        ...
                        <div className="flex flex-col gap-1">
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-sm text-slate-200">{req.fullName}</span>
                          ...
                          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px] text-slate-400">
                            <span>
                              Rank: <strong className="text-amber-300">{req.assignedRank}</strong>
                            </span>
                            <span>
                              Gender: <strong className="text-slate-300">{req.gender}</strong>
                            </span>
                            <span>
                              Roblox:{' '}
                              <strong className="text-sky-300">
                                @{req.robloxUsername} (ID: {req.robloxUserId})
                              </strong>
                            </span>
                            <span>
                              Citizen: <code className="text-slate-300">{req.targetUser.discordTag}</code>
                            </span>
                          </div>

                          {req.status === 'DECLINED' && req.declineReason && (
                            <div className="mt-1 text-[11px] text-rose-300 bg-rose-950/40 p-1.5 rounded border border-rose-500/30">
                              <strong>Rejection Reason:</strong> {req.declineReason} (Reviewed by{' '}
                              {req.reviewedBy?.discordTag || 'Staff'})
                            </div>
                          )}

                          {req.status === 'APPROVED' && req.issuedSerialId && (
                            <div className="mt-1 text-[11px] text-emerald-300 bg-emerald-950/40 p-1.5 rounded border border-emerald-500/30">
                              <strong>Serial ID Issued:</strong> <code className="font-mono">{req.issuedSerialId}</code> (Approved by{' '}
                              {req.reviewedBy?.discordTag || 'Staff'})
                            </div>
                          )}
                        </div>
                      ...
                      {req.status === 'PENDING' && (
                        <div className="flex items-center gap-2 shrink-0 self-end md:self-center">
                          <button
                            ...
                            className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white font-semibold text-xs rounded-lg flex items-center gap-1.5 cursor-pointer transition-colors shadow-sm"
                          >
                            <Check className="w-3.5 h-3.5" />
                            <span>Accept & Issue</span>
                          </button>
                          <button
                            onClick={() => setDeclineModal({ id: req.id, name: req.fullName })}
                            disabled={actingRequestId === req.id}
                            className="px-3 py-1.5 bg-rose-600/20 hover:bg-rose-600/30 border border-rose-500/40 text-rose-300 font-semibold text-xs rounded-lg flex items-center gap-1.5 cursor-pointer transition-colors"
                          >
      ...
      {declineModal && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 max-w-md w-full shadow-2xl flex flex-col gap-3">
            <div className="flex items-center justify-between border-b border-slate-800 pb-2">
              <div className="font-bold text-sm text-rose-300 flex items-center gap-2">
                <XCircle className="w-4 h-4 text-rose-400" />
                <span>Decline Application: {declineModal.name}</span>
              </div>
              <button
            ...
            <p className="text-xs text-slate-300 leading-relaxed">
              Please state the reason for rejection. This reason will be logged into the audit ledger and sent to the applicant.
            </p>
            ...
            <textarea
              ...
              rows={3}
              placeholder="e.g. Unverified Roblox account, incorrect rank selected, troll request..."
              className="w-full bg-slate-950 border border-slate-800 rounded p-2.5 text-xs text-slate-200 focus:outline-none focus:border-rose-500 font-sans"
            />
            ...
            <div className="flex items-center justify-end gap-2 pt-2">
              ...
              <button
                onClick={() => handleDeclineRequest(declineModal.id, declineReasonInput)}
                disabled={!declineReasonInput.trim()}
                className="px-4 py-1.5 bg-rose-600 hover:bg-rose-500 disabled:opacity-50 text-white rounded font-bold text-xs flex items-center gap-1.5 cursor-pointer shadow-sm"
              >
                <XCircle className="w-3.5 h-3.5" />
      ...
      {activeTab === 'generate' && (
        <div className="flex flex-col gap-3">
          {/* New Workflow Informational Banner */}
          <div className="bg-indigo-950/40 border border-indigo-500/50 rounded-lg p-3 text-xs text-indigo-200 flex items-start gap-2.5">
            <Inbox className="w-4 h-4 text-indigo-400 shrink-0 mt-0.5" />
            <div className="flex-1 flex flex-col gap-0.5">
              <div className="font-bold text-indigo-200">New Staff Approval Workflow</div>
              <p className="text-[11px] text-indigo-300/90 leading-relaxed">
                When citizens run <code className="bg-indigo-900/60 px-1 py-0.5 rounded text-indigo-100 font-mono">/card generate</code>, their card is <strong>not</strong> issued immediately. It is forwarded to your configured staff channel (<code className="font-mono text-amber-300">#{staffChannelInput || 'staff-chat'}</code>) for authorized officer verification with <strong>Accept</strong> and <strong>Decline (with reason)</strong> buttons.
              </p>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div className="bg-slate-950 p-3 rounded-lg border border-slate-800 flex flex-col justify-between">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] text-slate-500 font-mono">POST /api/card/request/submit</span>
              </div>
              <p className="text-xs text-slate-400 my-1 leading-relaxed">
                <strong>Citizen Submission Flow:</strong> Dispatches the application to <code className="text-amber-400 font-mono">#{staffChannelInput || 'staff-applications'}</code>. Staff can review, accept with 1-click, or decline with a required reason.
              </p>

              <div className="flex flex-col gap-2 mt-2">
                <button
                  onClick={handleSubmitCardRequest}
                  className="w-full py-2.5 px-3 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-xs rounded-lg flex items-center justify-center gap-2 cursor-pointer transition-colors shadow-sm"
                >
                  {issueSuccess ? (
                    <>
                      <Check className="w-4 h-4 text-emerald-300" />
                      <span>Application Forwarded to Staff Review Queue!</span>
                    </>
                  ) : (
                    <>
                      <Inbox className="w-4 h-4" />
                      <span>Submit Application (Routes to Staff Queue)</span>
                    </>
                  )}
                </button>

                <div className="flex items-center justify-between gap-2 pt-1 border-t border-slate-800/80">
                  <span className="text-[11px] text-slate-400">Officer direct override:</span>
                  <button
                    onClick={handleIssueCard}
                    className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-slate-300 text-[11px] rounded font-medium cursor-pointer transition-colors"
                  >
                    Direct Issue (Bypass Queue)
                  </button>
                </div>
              </div>
            </div>
